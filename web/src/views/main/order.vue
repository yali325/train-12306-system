...
import SessionStorage, { SESSION_ORDER } from "@/utils/session-storage";
import { PASSENGER_TYPE_ARRAY as ALL_PASSENGER_TYPE_ARRAY, SEAT_COL_ARRAY as ALL_SEAT_COL_ARRAY, SEAT_TYPE } from "@/utils/enums";
...
export default defineComponent({
  ...
  setup() {
    ...
    // }
    const tickets = ref([]);
    const PASSENGER_TYPE_ARRAY = ALL_PASSENGER_TYPE_ARRAY;
    ...
    const SEAT_COL_ARRAY = computed(() => {
      return ALL_SEAT_COL_ARRAY.filter(item => item.type === chooseSeatType.value);
    });
    // 选择的座位
    // {
